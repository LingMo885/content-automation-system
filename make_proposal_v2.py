...
p = doc.add_paragraph()
p.add_run('扬州支队现用红门电动门（K300G+HP30）故障需更换。我司受邀提交替代方案。')
...
p = doc.add_paragraph()
p.add_run('全套含门扇、立柱、电机、悬浮配件、车辆识别系统，包安装。方案相比红门原报价更低、颜色更符合军用标准。')
...
# ============ 第四部分：产品图纸 ============
add_heading(doc, '四、产品图纸', 1)

p = doc.add_paragraph()
p.add_run('我司设计图纸（深灰色竖条百叶6扇门）：')

# 插入设计图（ebe0bd48）
img1_path = '/Users/yyf/.openclaw/media/inbound/ebe0bd48-110b-4a7b-98f0-f2c1bc662395.png'
if os.path.exists(img1_path):
    doc.add_picture(img1_path, width=Inches(5.5))
...
# 插入门扇照片（4af56a6f）
img2_path = '/Users/yyf/.openclaw/media/inbound/4af56a6f-4ea4-44b2-b693-f200fc438066.png'
if os.path.exists(img2_path):
    doc.add_picture(img2_path, width=Inches(5.5))
    last_para = doc.paragraphs[-1]
    last_para.alignment = WD_ALIGN_PARAGRAPH.CENTER
...
# ============ 第六部分：供货与售后 ============
add_heading(doc, '六、供货周期', 1)
p = doc.add_paragraph()
run = p.add_run('合同签订后 15个工作日 内完成制作，现场安装 1个工作日，合计 16个工作日。')

add_heading(doc, '七、付款方式', 1)
pays = [
    '合同签订付 30% 定金（¥ 13,500）',
    '制作完成发货前付 40%（¥ 18,000）',
    '安装验收合格后付 30%（¥ 13,500）',
...
# ============ 联系信息 ============
add_heading(doc, '九、联系信息', 1)
tbl6 = doc.add_table(rows=4, cols=2)
tbl6.style = 'Table Grid'
contact_data = [('公司名称', '江苏武荣装备科技有限公司'), ('联系人', '【请填写】'), ('联系电话', '【请填写】'), ('邮    箱', '【请填写】')]
...
# 保存
output_docx = '/Users/yyf/Desktop/【投标项目】/扬州支队项目/扬州支队大门更换方案_江苏武荣_v3.0.docx'
